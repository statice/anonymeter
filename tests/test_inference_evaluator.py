...
from anonymeter.evaluators.inference_evaluator import \
    evaluate_inference_guesses
from anonymeter.evaluators.inference_evaluator import InferenceEvaluator


@pytest.mark.parametrize("guesses, secrets, expected",
                         [(('a', 'b'), ('a', 'b'), (True, True)),
...
def test_evaluate_inference_guesses_classification(guesses, secrets, expected):
    out = evaluate_inference_guesses(guesses=pd.Series(guesses),
                                     secrets=pd.Series(secrets),
                                     regression=False)
...
@pytest.mark.parametrize("guesses, secrets, expected",
                         [((1., 1.), (1., 1.), (True, True)),
                          ((1.01, 1.), (1., 1.01), (True, True)),
                          ((1., 1.), (2., 1.01), (False, True)),
                          ((1., 2.), (2., 1.01), (False, False))])
...
@pytest.mark.parametrize("guesses, secrets, tolerance, expected",
                         [((1., 1.), (1.05, 1.06), 0.05, (True, False)),
                          ((1., 1.), (1.05, 1.06), 0.06, (True, True)),
                          ((1., np.nan), (1.05, np.nan), 0.06, (True, True)),
                          ((np.nan, np.nan), (np.nan, np.nan), 0.06,
                           (True, True)),
                          ((1, np.nan), (np.nan, 1.06), 0.06, (False, False)),
                          ((1., 1.), (1.05, 1.06), 0.04, (False, False)),
                          ((1., 1.), (1.25, 1.26), 0.2, (False, False)),
                          ((1., 1.), (1.26, 1.25), 0.25, (False, True))])
