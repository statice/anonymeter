...
def test_scaling_numerical():
    ...
    tdf_ori, tdf_syn = mixed_types_transform(df_ori,
                                             df_syn,
                                             num_cols=['c'],
                                             cat_cols=[])
...
@pytest.mark.parametrize(
    ...
    [(pd.DataFrame({'c': ['a', 'b', 'c', 'd']
      ...
      pd.DataFrame({'c': [0, 1, 2, 3]}), pd.DataFrame({'c': [0, 1, 2, 2]})),
     (pd.DataFrame({'c': ['a', 'b', 'c', None]
                    }), pd.DataFrame({'c': ['a', 'b', 'c', 'c']}),
      pd.DataFrame({'c': [0, 1, 2, 3]}), pd.DataFrame({'c': [0, 1, 2, 2]})),
     (pd.DataFrame({'c': ['a', 'b', 'c', 'd']
                    }), pd.DataFrame({'c': ['a', 'b', None, 'c']}),
      pd.DataFrame({'c': [0, 1, 2, 3]}), pd.DataFrame({'c': [0, 1, 4, 2]}))])
def test_encoding_categorical(df1, df2, exp1, exp2):
    enc1, enc2 = mixed_types_transform(df1=df1,
                                       df2=df2,
                                       cat_cols=['c'],
                                       num_cols=[])
...
@pytest.mark.parametrize(
    "df1, df2, exp1, exp2",
    [(pd.DataFrame({'c': ['a', 'b', 'c']}), pd.DataFrame({
        'c': ['a', 'b', 'd']
    }), pd.DataFrame({'c': [0, 1, 2]}), pd.DataFrame({'c': [0, 1, 3]})),
     (pd.DataFrame({'c': ['a', 'b', 'c']}),
      pd.DataFrame({'c': ['a', 'b', None]}), pd.DataFrame(
          {'c': [0, 1, 2]}), pd.DataFrame({'c': [0, 1, 3]})),
     (pd.DataFrame({'c': [None, 'b', 'c']}),
      pd.DataFrame({'c': ['a', 'b', None]}), pd.DataFrame(
          {'c': [3, 1, 2]}), pd.DataFrame({'c': [0, 1, 3]}))])
