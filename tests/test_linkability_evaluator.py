...
from anonymeter.evaluators.linkability_evaluator import \
    LinkabilityEvaluator
from anonymeter.evaluators.linkability_evaluator import LinkabilityIndexes
...
@pytest.mark.parametrize(
    ...
    [(1, 0, 0.25, (0.25, 0.25)), (2, 0, 1, (1., 1.)), (3, 0, 1, (1., 1.)),
     (4, 0, 1, (1., 1.)), (1, 0.95, 0.3725, (0.045587, 0.699358)),
     (2, 0.95, 0.7551, (0.5102, 1.))])
def test_linkability_evaluator(n_neighbors, confidence_level, expected_risk,
                               expected_ci, n_attacks):
    ori = pd.DataFrame({'col0': [0, 0, 4, 0], 'col1': [0, 1, 9, 4]})
    syn = pd.DataFrame({'col0': [0, 1, 4, 9], 'col1': [0, 1, 4, 9]})
    # Nearest neighbors in order. In the following schema,
    # 'O' refers to original, 'S' to synthetic. The first digit
    # indicates the row and the second the column, so 'O12' is
    # equivalent to ori.iloc[1, 2].
    # O00 -> S00, S01, S02, S03
    # O10 -> S10, S11, S12, S13
    # O01 -> S00, S01, S02, S03
    # O11 -> S11, S10, S12, S13
...
@pytest.mark.parametrize(
    ...
    [(1, 0, 0.25, (0.25, 0.25)), (2, 0, 1, (1., 1.)), (3, 0, 1, (1., 1.)),
     (4, 0, 1, (1., 1.)), (1, 0.95, 0.3725, (0.045587, 0.699358)),
     (2, 0.95, 0.7551, (0.5102, 1.))])
def test_linkability_evaluator_neighbors(n_neighbors, confidence_level,
                                         expected_risk, expected_ci,
                                         n_attacks):
...
@pytest.mark.parametrize('n_neighbors, fails', [(1, False), (2, False),
                                                (3, False), (4, False),
                                                (5, True), (45, True)])
...
@pytest.mark.parametrize('n_neighbors, expected_risk', [(1, 0.25), (2, 5 / 6),
                                                        (3, 1), (4, 1)])
def test_baseline(n_neighbors, expected_risk):
    ...
    ori = pd.DataFrame(rng.choice(["a", "b"], size=(400, 2)),
                       columns=['c0', 'c1'])
    syn = pd.DataFrame([["a", "a"], ["b", "b"], ["a", "a"], ["a", "a"]],
                       columns=['c0', 'c1'])
    evaluator = LinkabilityEvaluator(ori=ori,
                                     syn=syn,
                                     n_attacks=None,
...
def test_linkability_risk(confidence_level):
    ...
    evaluator = LinkabilityEvaluator(ori=ori,
                                     ...
                                     n_attacks=10,
                                     n_neighbors=5,
                                     aux_cols=(col_sample[:2], col_sample[2:]))
    evaluator.evaluate(n_jobs=1)
    risk, ci = evaluator.risk(confidence_level=confidence_level)
...
def test_evaluator_not_evaluated():
    evaluator = LinkabilityEvaluator(ori=pd.DataFrame(),
                                     syn=pd.DataFrame(),
                                     aux_cols=[])
