...
from anonymeter.evaluators.singling_out_evaluator import safe_query_counts
from anonymeter.evaluators.singling_out_evaluator import \
...
def test_so_general(mode):
    ...
    soe = SinglingOutEvaluator(ori=ori, syn=syn,
                               n_attacks=5).evaluate(mode=mode)
...
@pytest.mark.parametrize('query, result', [("c1 == 0 and c2 == 'a'", 2),
                                           ("c3 == 'fuffa'", None),
                                           ("c1 == 2 and c2 == 'c'", 1)])
...
def test_singling_out_query_generator():
    ...
    possible_queries = [
        'c1<= 1.23 & c1>= 9.87', "c1<= 1.23 & c0== 'b'",
        "c1<= 1.23 & c0== 'a'", 'c1>= 9.87 & c1<= 1.23',
        "c1>= 9.87 & c0== 'b'", "c1>= 9.87 & c0== 'a'", "c0== 'b' & c1<= 1.23",
        "c0== 'b' & c1>= 9.87", "c0== 'b' & c0== 'a'", "c0== 'a' & c1<= 1.23",
        "c0== 'a' & c1>= 9.87", "c0== 'a' & c0== 'b'"
    ]
...
@pytest.mark.parametrize("w_min, w_max", [(0, 1), (1 / 10000, 1 / 1000),
                                          (0.0013414, 0.2314)])
def test_probability_integral(n, w_min, w_max):
    def _so_probability(n: int, w: float):
        return n * w * ((1 - w)**(n - 1))

    desired, _ = integrate.quad(lambda x: _so_probability(w=x, n=n),
                                a=w_min,
                                b=w_max)
