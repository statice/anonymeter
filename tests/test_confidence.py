...
from anonymeter.stats.confidence import bind_value
from anonymeter.stats.confidence import EvaluationResults
from anonymeter.stats.confidence import probit
from anonymeter.stats.confidence import residual_success
from anonymeter.stats.confidence import success_rate
from anonymeter.stats.confidence import SuccessRate
...
@pytest.mark.parametrize('n_success, expected_risk, expected_error', [
    (850, 0.849, 0.022),
    (0, 0.002, 0.002),
    (1000, 0.998, 0.002),
])
def test_success_rate(n_success, expected_risk, expected_error):
    rate, error = success_rate(n_total=1000,
                               n_success=n_success,
...
@pytest.mark.parametrize('attack_rate, control_rate, expected', [
...
def test_residual_success(attack_rate, control_rate, expected):
    residual = residual_success(attack_rate=attack_rate,
                                control_rate=control_rate)
    np.testing.assert_equal(np.round(residual, decimals=3), expected)


@pytest.mark.parametrize('point_estimate, error_bound, expected', [
    (0.1, 0.3, (0.1, 0.0, 0.4)),
    (1.1, 0.5, (1.0, 0.6, 1.0)),
    (-0.1, 0.2, (0.0, 0.0, 0.1)),
])
def test_bind_value(point_estimate, error_bound, expected):
    ...
    np.testing.assert_almost_equal(
        np.array([risk.value, risk.ci[0], risk.ci[1]]), expected)


@pytest.mark.parametrize('n_attacks, n_success, n_baseline', [(100, 100, 0),
                                                              (100, 23, 11),
                                                              (111, 84, 42),
                                                              (100, 0, 100)])
def test_evaluation_results_simple(n_attacks, n_success, n_baseline):
    results = EvaluationResults(n_attacks=n_attacks,
                                n_success=n_success,
...
def test_evaluation_results_confidence(n_attacks, n_success, n_baseline,
                                       n_control, confidence_level,
                                       expected_rate, expected_baseline):
    results = EvaluationResults(n_attacks=n_attacks,
                                n_success=n_success,
                                n_baseline=n_baseline,
                                n_control=n_control,
    ...
    np.testing.assert_equal(results.risk(baseline=False),
                            expected_rate.to_risk())
    np.testing.assert_equal(results.risk(baseline=True),
                            expected_baseline.to_risk())


def test_evaluation_results_warns_baseline():
    with pytest.warns(UserWarning):
        EvaluationResults(n_attacks=100,
                          n_success=49,
                          n_baseline=50,
                          n_control=None,
                          confidence_level=0.95)
